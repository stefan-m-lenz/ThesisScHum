...
\subsubsection{Types of Boltzmann machines}\label{bmtypes}
...
\end{align}
In equations (\ref{eqn:derivedlog}) and (\ref{eqn:derivedexp}), the derivative of the logarithm and the exponential function is used, respectively. The resulting terms can be rewritten in equation (\ref{eqn:nablaresult}) as the expectation of the gradient given the distribution of the data and the expectation of the gradient given the distribution of the model.


As can be seen in equation (\ref{eqn:derivedexp}), the calculation of the gradient involves sums over lots of possible combinations of activations of nodes. In the first term, the sum goes over all combinations of hidden nodes, in the second over all nodes in the network.
Calculating the gradient and is technically not feasible in normal cases, as the number of summands grows exponentially with the number of nodes.
This means that it is not possible to simply equate the gradient of the log-likelihood with zero and solve the equation to get an optimum. Also finding an optimum with gradient descent, walking with small steps in the direction of the gradient to find an optimum, is not possible directly, because this would as well require the calculation of the gradient in each iteration step.

TODO stochastic gradient descent update term.

In an RBM with Bernoulli distributed nodes, this leads to the following that 
\begin{align}
- \frac{\partial}{\partial w_{ij}}  \log p(\widetilde{v}) &= \EX_{P_\text{data}}  \widetilde{v}^T h - \EX_{P_\text{model}} v^T h \\
- \frac{\partial}{\partial a_{ij}}  \log p(\widetilde{v}) &=  \EX_{P_\text{data}} 
\widetilde{v} - \EX_{P_\text{model}}  v \\
- \frac{\partial}{\partial b_{ij}}  \log p(\widetilde{v}) &=  \EX_{P_\text{data}} h - \EX_{P_\text{model}} h 
\end{align}
...
The complexity for calculating the partition function for all the different types of  models described in section \ref{bmtypes} is then still $\mathcal{O}(2^n)$, but with an $n$ smaller than the number of nodes:
...
\caption{Possibilities for summing out layers for calculating the likelihood. For calculating the likelihood in a more efficient way, the layers in the gray boxes can be summed out analytically. {\bf A:} Summing out the odd layers in this DBM results in $2^{(4 + 4)} = 256$ summands, {\bf B:} Summing out the even layers in this DBM results in $2^{(4 + 3)} = 128$ summands, {\bf C:} Summing out the even layers in  this multimodal DBM results in $2^{(6 + 3)} = 512$ summands.}
\label{aissummingout}
\end{figure}
...
Like described by Salakhutdinov et al. \citep{salakhutdinov2008learning}, it is possible to use only the states of each second layer from samples generated by running a Gibbs chain. The unnormalized probabilities of these states can be calculated by analytically summing out the other layers.
In a multimodal DBM, we can use the fact that the states in one layer are independent from the states of each non-adjacent layer to get $p^*(\tilde{x}) = p^*(h^{(1)}) \cdot p^*(h^{(3)}) \cdot \dots $ as unnormalized probability for the subset $\tilde{x} = (h^{(1)}, h^{(3)}, \dots)$ of a generated sample $x = (v, h^{(1)}, h^{(2)}, \dots)$.
...
This procedure can generalized for AIS on multimodal DBMs. 
If we have the unnormalized probability $p^*(h)$ for each type of RBM that receives the data input, it becomes possible to calculate the unnormalized probability of sampled hidden values in a multimodal DBM in the same ways as for a standard DBM with only Bernoulli distributed nodes.

To be able to do this, it is necessary to have formulas for $p^*(h)$ for each different type of RBMs. These can be derived by analytically ``summing out", or in case of Gaussian RBMs, ``integrating out" the visible layer.

\paragraph{Derivation of unnormalized probability for different types of restricted Boltzmann machines}
...
TODO nochmal recherchieren was andere Methoden angeht: variational autoencoder, GANs 
