...
There are also other weaknesses of $k$-anonymity, which are targeted by proposed refinements, which introduce additional parameters for indicating the privacy level \citep{machanavajjhala_l-diversity_2007, li_t-closeness:_2007}.
...
Synthetic data can be generated with generative models.
TODO VAE \citep{Kingma2013}, GAN \citep{goodfellow_generative_2014}

 MICE \citep{goncalves}
 
The algorithm for fitting a MICE model consists of the following steps \citep{goncalves, dsdbm}:
\begin{enumerate}
...
\item Estimate the empirical distribution $\hat{p}_1$ of $v_1$. For a binary variable, this means calculating the mean value $\hat{\mu}_1$.
\item
\end{enumerate}

From the MICE model $(\hat{\mu}_1, R_2, \dots, R_n)$, new samples $\tilde{v}_1, \dots , \tilde{v}_n$ can be generated:

In the case of Bernoulli of  case, this means that the mean value $\hat{\mu}_1$ of each variable $v_i$ to used to sample $\tilde{v}_1$ from a Bernoulli distribution with  $p(\tilde{v}_1 = 1) = \hat{\mu}_1$.

As simplest model we use the method of independent marginals (IM) like \cite{goncalves}.
...
For Bernoulli distributed variables, this means to sample using their estimated marginal means, as described above in the algorithm of the MICE method, where this is used for sampling the first variable.
With this, the IM method provides a baseline comparison as it does not aim at modeling the correlation structure at all.
...
For the experiments here, the differences of pairwise log odds ratios is utilized as as performance metric \citep{nussberger_synthetic_2020}.
...
We do not modify the training procedures of the generative models.
Instead, we want to measure the disclosure of the generated data directly and compare the different approaches with respect to disclosure.

Overfitting, membership attack

%TODO working with synthetic data principle
...
The models with the best performance are chosen.
To ensure a fair comparison, the pick of the best model for each approach uses the same performance measure that is used for comparing the different approaches.
In GANs it is the only option to use a metric on the generated data because they do not have a single performance criterion like the lower bound of the likelihood as discriminator and generator in a GAN are optimized independently.
Here we use the log odds ratios as described in \ref{measuringperformance} for comparing the models and for picking the model with the best performance for each approach.
...
%TODO zweiter Teil der Grafik mit distributed generative approach
