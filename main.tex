...
\usepackage[a4paper,bindingoffset=0.2in,%
            left=2.8cm,right=2.8cm,top=2.1cm,bottom=2.8cm]{geometry}
\usepackage{helvet}
\usepackage{pdfpages}
...
\lstdefinelanguage{Julia}
  {morekeywords={begin,abstract,break,case,catch,const,continue,do,else,elseif,
      ...
      macro,module,otherwise,quote,return,switch,true,try,type,
      using,while},
   sensitive=true,
   alsoother={$},
   morecomment=[l]\#,
...
\subsubsection{Generalizing training and evaluation of multimodal DBMs}
...
The evaluation via AIS can also be generalized, using the unnormalized probabilities for the different RBMs at the input layer (see \ref{unnormalizedprobsrbm}) to ``sum out" or ``integrate out" the different types of visible nodes (see figure \ref{aissummingout} C).

\paragraph{Annealed importance sampling for multimodal DBMs}
...
TODO Einleitung after evaluating Julia package .
...
\hline
\end{tabular}
\caption{\label{tab:FeatureOverviewBoltzmann} Comparison of features between different implementations of restricted and deep Boltzmann machines.}
\end{table}


The implementation of \cite{theis2011deepbelief} (\apkg{deepbelief}), and TODO \apkg{darch} \citep{darch},
No learning of $\sigma$ in other implementations. Difficult anyway.
% pseudo-likelihood: https://scikit-learn.org/0.23/modules/generated/sklearn.neural_network.BernoulliRBM.html

The Julia package \apkg{Boltzmann} existed before the \apkg{BoltzmannMachines} package. 
At the time of the decision whether to start a new implementation, the \apkg{Boltzmann} package was very much in development and did not have the desired features, so it seemed to be simpler to start with a clean slate and implement the desired features in a new package that was designed for training DBMs. In the current version, the \apkg{Boltzmann} implements algorithms for training RBMs and DBNs. The algorithm for training deep Boltzmann machines is not implemented in the \apkg{Boltzmann} package as of version 0.7.
The only measure for evaluating RBMs\footnote[1]{\url{https://github.com/dfdx/Boltzmann.jl/blob/0a1848a190c4cd7aa2ae3c0f6b6fa83ceac1613e/src/rbm.jl\#L169}} provided there is the pseudo-likelihood (see \cite{goodfellow_deep_2016}, chapter 18.3).
...
The \apkg{deepbelief} package \citep{deepbelief_github} provides a very clean, object-oriented implementation of RBMs and DBNs in Python, using the linear algebra library \apkg{NumPy} \citep{numpy}.
...
It also provides an implementation of binary RBMs and Gaussian RBMs as described by \cite{krizhevsky2009tinyimages}.
It also provides implementations for AIS in both RBMs with only Bernoulli distributed nodes and Gaussian RBMs.
Training and evaluation of DBMs is not provided.

AIS in yell not generic \footnote[1]{\url{https://github.com/yell/boltzmann-machines/blob/93ece3497816ccf08f687e3237007544268788e7/boltzmann_machines/dbm.py\#L902}}
...
The \apkg{darch} package uses the GNU general public license version 3 (GPL-3,  \cite{gplv3}), which does not allow commercial derivations of the code.


%TODO Tensorflow.jl

%TODO TensorFlow, Flux, Pytorch etc. warum nicht diese verwenden
...
With the function \inlinecode{mul!} instead of the normal matrix multiplication operator (\inlinecode{*}), it is possible to perform matrix multiplication in-place, without allocating space for the result. Avoiding the costs of memory allocations and garbage collection greatly increased the speed in many parts of the code.
...
% TODO bei Diagramm Softmax0 einbauen
\begin{landscape}
\begin{figure}[h]
   \centering
...
\subsubsection[Usage of the BoltzmannMachines Julia package]{Usage of the \apkg{BoltzmannMachines} Julia package}
