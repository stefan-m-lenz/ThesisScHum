...
The likelihood and the variational lower bound of the likelihood can be estimated via the annealed importance sampling (AIS, see Section \ref{evaluatingbms}). 
...
Therefore it is an important asset if the algorithms are also available in R.
The \apkg{JuliaConnectoR} package also provides a 
The possibility for importing complete packages into R is a 


For the use in a setting with distributed sensitive data, the \apkg{BoltzmannMachines} package can be wrapped into an DataSHIELD R package.
The result is the first package to implement deep learning in DataSHIELD.
This can be achieved via the \apkg{JuliaConnectoR}.
