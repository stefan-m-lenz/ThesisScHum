...
A deep Boltzmann machine with $n_L$ number of hidden layers has the parameters $\Theta = (W^{(1)}, \dots, W^{(n_L)}, a, b^{(1)}, \dots, b^{(n_L)})$. The energy is defined as 
\[
E(v, h^{(1)}, \dots, h^{(n_L)}) = - a^T v - v^T W^{(1)} h^{(1)} - \sum_{k=1}^{n_L} b^{(k)} h^{(k)} -  \sum_{k=2}^{n_L} h^{(k-1)}W^{(k)}h^{(k)}
\]
For the resulting network architecture see figure  \ref{fig:bmsoverview}{\bf C}.
