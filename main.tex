...
Similar to \cite{krizhevsky2009tinyimagesthesis} let us write $E(v_{i \in C_k} = 0, v_{i \notin C_k}, h)$ for the energy of the combination of a visible vector $v$ that is zero in all dummy variables that belong to $C_k$ and a hidden vector $h$.
%If one dummy variable in the set is one, all others are in the set are zero, therefore:
%\begin{equation}
%E(v_k=1, v_{i\neq k}, h) = E(v_k = 1, v_{i \notin C_k}, h).
%\end{equation}
