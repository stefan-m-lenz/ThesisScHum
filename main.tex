...
From the previously defined dummy encoding results that if one dummy variable in the set is one, all others are in the set are zero. 
The sum of the energies can therefore be split into those parts where a dummy variable $k$ is zero and one part where all others are one, because these covers all allowed combinations.
...
With this notation defined, the conditional probability of a dummy variable being one can be derived as follows:
