...
For this purpose, DBMs are compared with VAEs, GANs, and simpler approaches, such as multiple imputation by chained logistic regression models \citep{mice}, with respect to generative performance and the disclosure risk of synthetic data that has been generated by these approaches (Section \ref{realexp}).
...
Therefore it is an important asset if the algorithms are also available to R users.
The \apkg{JuliaConnectoR} offers a generic solution to import functionality from Julia packages into R (see Section \ref{juliaconnectoRAutoimport}), which is not limited to the \apkg{BoltzmannMachines} package.
...
Furthermore, the \apkg{Julia\-ConnectoR} facilitates the interactive use by allowing to interrupt call to Julia without threatening the stability of the R session.
...
This allows to use create synthetic data for the use in a setting with distributed data that is subject to data protection constraints.
The resulting solution is also the first approach that enables deep learning in DataSHIELD.
