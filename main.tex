...
Conditional sampling is conveniently possible via this function (see p.\ \pageref{bms_samples}).
...
Assuming here that \inlinecode{x} has only 6 variables, the code in \ref{lst:partdbm} results in a model shown in figure \ref{fig:smallpartitioneddbm}.
...
The optimization procedure is also customizable. 
...
Monitoring the pre-training of layers individually is also possible via the \inlinecode{TrainLayer} mechanism.
