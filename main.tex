...
\begin{table}[h!]
\centering
\begin{tabular}{p{4.4cm} p{1.1cm} p{1.1cm} p{1.1cm} p{1.1cm} p{1.1cm} p{1.1cm}}
\hline \\ [3.9ex]\\
...
\hline
\end{tabular}
\caption{\label{tab:FeatureOverview} Comparison of features between different implementations of restricted and deep Boltzmann machines
\\ (*) In the \apkg{Boltzmann} package, the formulas for the Gaussian RBM are not documented and the implementation does not match the formulas described here.}
\end{table}
...
It was very much in development and did not have the desired features, so it seemed to be simpler to start with a clean slate and implement the desired features in a new package that was designed for training DBMs and not only RBMs.
In particular, the algorithm for training deep Boltzmann machines is still not implemented in the \apkg{Boltzmann} package as of version 0.7.

The package of \cite{theis2011deepbelief}
