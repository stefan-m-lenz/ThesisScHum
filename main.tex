...
In the following, we take a closer look how the \apkg{BoltzmannMachines} Julia package handles the different algorithms and all their different variants.
...
Instead of using classical object oriented inheritance such as in Java or C++, inheritance relationships (triangle-shaped arrows in the UML diagram) can be implemented in Julia using {\em multiple dispatch} of the functions.
...
\begin{figure}
   \centering
   \includegraphics[scale=0.5,trim={5.3cm 4cm 6.8cm 3.4cm},clip,angle=-90]{images/BoltzmannMachinesDiagram.pdf}
...
The functions that are available for all different subtypes can be seen in \ref{umlclassdiagram} in the box below the type name.
...
The training is handled separately for DBMs and RBMs via the functions \inlinecode{fitrbm} and \inlinecode{fitdbm}, which can be used for fitting an RBM or DBM model, respectively. 
These functions get as input a data set and all hyperparameters for training the model and return the trained model.
the function \inlinecode{traindbm!} performs the 
...
This design a very flexible composition of DBMs.
In the diagram this is indicated by the composition relationship (line with filled diamond).
...
For example, summing out of nodes can be done more flexibly and effectively in DBMs with only Bernoulli distributed nodes (see figure \ref{aissummingout}).
(Multimodal) deep Boltzmann machines are constructed using restricted Boltzmann machines as building blocks.
Multimodal DBMs correspond to the type \inlinecode{MultimodalDBM}.
This is a vector of RBMs or  \inlinecode{PartitionedRBMs} can be used to combine different RBMs, which may have different types, to build stacks of partitioned RBMs that form (see figure \ref{mdbmimplasstack}).
...
Different types of RBMs modeled as Julia composite types. 
...
%TODO hiddenpotential: deterministic value, here expected value see \ref{reconstructionerror}
% The expected value is used for contrastive divergence.
% scaling for dbm pre-training
As recommended by \citep{hinton_practical_2012} (section 3.2 there), the probabilities are used for the visible states in the last step of sampling in the model distribution.
