...
It is also not possible to infer the type automatically, because it is possible that the results of different methods, which are invoked for different argument types, differ completely.
...
Consider the example in Listing \ref{multdispatchex}, which illustrates this.
TODO explain

\begin{lstlisting}[language=Julia, float=!h, numbers=left, caption={Multiple dispatch and type translation}, label={multdispatchex}]
R> juliaEval("f(i::Int) = i*2")
R> juliaEval("f(i::Float64) = i*3")
R> juliaEval("f(i::Number) = i*4")
R> f <- juliaFun("f")
R> f(2L)
[1] 4
R> f(2)
