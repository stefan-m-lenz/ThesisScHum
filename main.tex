...
One of the main reasons for using a language bridge such as the \apkg{JuliaConnectoR} is to access functionality from packages that are only available in other languages while being able to do most of the work in a language that is more familiar or more suitable for the rest of the task.
An example for this can be to use deep learning in Julia but perform data preprocessing and plotting in R.

The function \inlinecode{juliaImport} (see also on p.~\pageref{rdokitem_juliaImport}ff.) of the \apkg{JuliaConnectoR} can inspect a Julia package and then return all Julia functions, including type constructors, in the form of an environment that contains corresponding R functions that wrap the Julia functions.
...
Via the \apkg{JuliaConnectoR} and \inlinecode{juliaImport}, the R code shown in Listing \ref{fluximportR} can do the same as the Julia code in Listing \ref{fluximportJulia}.

\begin{lstlisting}[language=R,float=!h,caption={R code using the \apkg{JuliaConnectoR} for  doing the same as in Listing \ref{fluximportJulia}},label={fluximportR}]
...
An alternative to importing a package in this way is to address the functions via their name as strings via the \inlinecode{juliaCall} function, e.g., by replacing
...
This is possible but it makes the code harder to read.
In the case of constructing the neural network in the last two lines of Listing \ref{fluximportR} this becomes obvious.
Replacing the imported functions with \inlinecode{juliaCall} there would deteriorate the legibility in a significant way.

An additional nice side benefit of using \inlinecode{juliaImport} is that the auto-complete functionality can aid in typing and finding functions that are collected in the returned environment.

A detail that is also interesting in Listing \ref{fluximportJulia} is that the activation function is specified via passing a function as an argument, which is here the \inlinecode{relu} function from the \apkg{Flux} package.
...
It is also possible to import local Julia modules into R in a similar way as shown above with Julia packages.
Wrapping code in local modules has the advantage TODO
For more details, including examples for using local modules, see the documentation of \inlinecode{juliaImport} (p. \pageref{rdokitem_juliaImport}ff.).

\subsubsection{Translating data between Julia and R}\label{translatejuliar}
Notable in Listing \ref{fluximportR} is that the numbers needed to be passed as integers.
This is because the \inlinecode{Dense} function in Julia expects integer values as arguments.
