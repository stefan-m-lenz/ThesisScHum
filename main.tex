...
One of the main reasons for using a language bridge such as the \apkg{JuliaConnectoR} is to access functionality from packages that are only available in other languages.
An example for this can be to use deep learning in Julia but perform data preprocessing and plotting in R.

The function \inlinecode{juliaImport} of the \apkg{JuliaConnectoR} can inspect a Julia package and then return all Julia functions, including type constructors, in the form of an environment that contains corresponding R functions that wrap the Julia functions.
This approach helps to handle foreign packages in a simple way, and it aids in writing more concise code, as demonstrated in the following.

At first, a minimal example of how the \apkg{Flux} Julia package can be employed in R  is shown in Listing \ref{fluximportJulia} and Listing \ref{fluximportR}.
...
This should demonstrate that this feature of the \apkg{JuliaConnectoR} can be used with any package.
In Listing \ref{fluximportJulia} and Listing \ref{fluximportR}, two packages are involved.
The first package that is imported into R is the Julia \inlinecode{Pkg} package.
This package is contained in the Julia standard library.
Its purpose is to install and manage other packages.
In this case, \inlinecode{Pkg} is used to install the \apkg{Flux} package in version 0.11.
...
This can make the hard to read.
The automatic import enables one to wrote conc 
A nice, small side benefit is that the auto-complete functionality can aid in typing and finding functions.


It is also possible to import local Julia modules into R similar to packages.
Wrapping code in local modules has the advantage 
...
Notable in Listing \ref{fluximport} is that the numbers needed to be passed as integers.
