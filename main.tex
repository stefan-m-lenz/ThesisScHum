...
Like the \apkg{Boltzmann} Julia package, it offers as evaluation metric the pseudo-likelihood.

The R package \apkg{darch} \citep{darch} offers a similar range of features as the \apkg{Boltzmann} package, covering the training of binary RBMs and DBNs.
...
It is also the only Julia package for training DBMs.
...
This feature is also used in the test suite of the \apkg{BoltzmannMachines} package for validating the AIS implementation by testing its results against the exact value in smaller models.
...
At some point, if the performance needs to be increased further,  it becomes necessary to switch to C for time-critical portions of the code and to call the C code using the MATLAB C interface.
...
In contrast to MATLAB or R, it is possible to gradually optimize the speed of code in Julia.
...
Instead it stores the result in a pre-existing matrix that is passed as an argument.
...
Julia offers the possibility to use parallelization via parallel processes very easily. This works also across different computers in a compute cluster without requiring the developer to change the code to adapt to a setting involving multiple computers.
...
In grid search \citep{bergstra_algorithms_2011} for example, many models can be trained completely in parallel in Julia, e.g., via using the Julia function  \inlinecode{pmap}, and the execution time of the search can therefore be divided by the number of processes available in the whole compute cluster.
...
The gradients for training RBMs and DBMs (see \ref{rbmgradients} and \ref{dbmtraining}), however, rely on Gibbs sampling and the mean-field algorithm.
...
\begin{figure}
   ...
   \caption{UML class diagram showing the type  relationships and the most important functions in the \apkg{BoltzmannMachines} Julia package
...
Thus the actual run-time type of objects is used to determine the method instead of the compile-time type.
...
For each of the Boltzmann machines models, it is possible to generate samples by using Gibbs sampling via the \inlinecode{samples} function.
Conditional sampling is conveniently possible using this function (see \ref{usagesampling} and p.~\pageref{bms_samples}).
...
\begin{tabularx}{\textwidth}{X}
 \Xhline{1pt}
   ...
   Fits a (multimodal) deep Boltzmann machine model for a given data set \inlinecode{x} and optional model hyperparameters \\
 \Xhline{1pt}
\end{tabularx}
\end{table}
...
In the UML diagram, this composition of DBMs from vectors of RBMs is indicated by the UML composition relationship, which is depicted as a line with a filled diamond at the end.
...
\begin{figure}[htb!]
   ...
   \caption{Multimodal deep Boltzmann machines are modeled as a stack of restricted Boltzmann machines. On the left hand side, an example model is depicted as a graph and on the right hand side the corresponding representation in the Julia package is shown.
   The first/lowest and the second/middle layer are modeled as \inlinecode{PartitionedRBM}s (white boxes with grey borders), each containing a vector holding two \inlinecode{BernoulliRBM}s (grey filled boxes).
...
\label{juliaFunTableEval}
 ...
 \Xhline{1pt}
   ...
   Calculates the log-likelihood of data \inlinecode{x} in an RBM. The log of the partition function can be provided as parameter \inlinecode{logz} or is estimated using AIS. \\
   \makecell[tl]{
   ...
   Calculates (logarithmized) AIS importance weights for annealing from the given model to a null model as shown in Figure \ref{figTwotypesais} A and B. Implicitly called by the functions above. \\
     \inlinecode{aislogimpweights(rbm1, rbm2; ...)}  \rightpageref{bms_aislogimpweights} \\
    Calculates AIS importance weights for annealing from one RBM model to another as shown in Figure \ref{figTwotypesais} C.
...
Optimizing the algorithms for DBMs that contain only Bernoulli distributed nodes such as the \inlinecode{BasicDBM} or the \inlinecode{PartitionedBernoulliDBM} is possible by implementing specialized methods for these types.
...
The methods listed for \inlinecode{AbstractRBM}s are implemented for all subtypes of RBMs in the package.
...
These variants are mostly used internally in the code of the package as they allow to reuse allocated space, which makes the implementation much faster.
...
\begin{lstlisting}[language=Julia, float = !h]
...
Analogously to how RBMs are arranged as building blocks in DBMs (see Figure \ref{mdbmimplasstack}), these hyperparameter definition objects can be arranged in vectors and passed to \inlinecode{fitdbm} or \inlinecode{stackrbms}.
Objects of type \inlinecode{TrainLayer} are then translated to RBMs, and objects of type \inlinecode{TrainPartitionedLayer} are translated to partitioned RBMs.
For illustrating this, let us assume here that \inlinecode{x} has only 6 variables.
Then the code in Listing \ref{lst:partdbm} results in a model with an architecture as shown in Figure \ref{fig:smallpartitioneddbm}.
Each \inlinecode{TrainLayer} object is used for training an RBM layer.
...
The gradients calculated from this input can be customized, e.g., to add a regularization term.
This can be done by creating a new subtype of \inlinecode{AbstractOptimizer} (see p.\ \pageref{bms_AbstractOptimizer}) and passing it to the training function or by specifying the optimizer in a \inlinecode{TrainLayer} object via the \inlinecode{optimizer} argument.


\paragraph{Different types of visible nodes}

For training RBMs or DBMs types with other types of visible nodes, such as those for data with variables that have more than two categories, the type of the RBM can be specified as argument \inlinecode{rbmtype} for the \inlinecode{fitrbm} function or for creating the corresponding \inlinecode{TrainLayer} object.
...
Synthetic data can be generated with the \inlinecode{samples} function, which expects a Boltzmann machine model and the number of samples as arguments.
...
Conditional sampling, here e.g., with the first and third variable being set to 1.0, and the second variable set to 0.0, can be also be done using the \inlinecode{samples} function:

\begin{lstlisting}[language=Julia]
samples(dbm, 500; conditions = [1 => 1.0; 2 => 0.0; 3 => 1.0])
\end{lstlisting}

For re-using allocated space during Gibbs sampling, there are the additional functions  \inlinecode{gibbssample!} and \inlinecode{gibbssamplecond!} (see p.\ \pageref{bms_gibbssample!}).
...
For each of the training functions in Table \ref{juliaFunTableTrain}, there is also a function prefixed with ``\inlinecode{monitoring\_}'', which allows to write the monitoring with functions such as those listed in Table \ref{monfun} in a compact way.
...
The monitoring functions are expected to write their results into the \inlinecode{Monitor} objects (see p.\ \pageref{bms_Monitor}) that are received as first argument of the monitoring functions.
These \inlinecode{Monitor} objects are collected and returned in form of a vector as the first result of the monitored training functions.
...
The plotting functionality has been separated in another package because plotting is not available in the base installation of Julia.
...
The main reason for developing new software was that the other packages did not work reliably.
One issue was run-time stability, which is important for software that needs to be deployed across a number of different sites and that has to function without human intervention.
...
Subsequently, in Section \ref{juliaconnectorDbmexample}, it is demonstrated how these features help to train and evaluate DBMs in R.
...
Another disadvantage of the coupling via a C-interface is that this requires that R is built as a shared/dynamic library.
...
In its default installation, R is not configured as a shared/dynamic library because this can reduce the performance of R by 10-20\% \citep{rAdmin}.
...
As JSON is text-based, numeric values have to be transformed from their binary form into decimal values and their textual representation has to be wrapped in a JSON message.
This additional overhead slows down the communication, in particular for larger matrices and vectors.
Therefore, \apkg{XRJulia} departs from the strategy of using TCP for these cases.
...
Although the mechanism for setting up such a remote connection is not yet exposed in the \apkg{JuliaConnectoR}, the design supports an integration of this functionality.
...
This may be used to employ remote servers that provide compute power but do not offer the same convenience as a local machine, e.g., if an integrated development environment such as RStudio \citep{rstudio} cannot be used on a remote server.
...
One of the main reasons for using a language bridge such as the \apkg{JuliaConnectoR} is to access functionality from packages in other languages while being able to do most of the work in a language that is more familiar to the user or more suitable for the rest of the task.
...
This approach helps to handle external packages in a simple way, and it aids in writing more concise code, as can be seen in the following.
...
This demonstrates that the \apkg{JuliaConnectoR} can be used for importing arbitrary packages.
...
An alternative to importing a package in this way is to address the functions via their name as strings with the \inlinecode{juliaCall} function of the \apkg{JuliaConnectoR} package (see p.~\pageref{rdokitem_juliaCall}). 
For example, lines 2 and 3 in Listing \ref{fluximportR} could be replaced with:
...
In the R code in Listing \ref{fluximportR}, this also becomes possible because the \inlinecode{relu} function has been imported as a function object and the \apkg{JuliaConnectoR} recognizes imported functions and can translate them back to Julia functions.
Passing functions as arguments to other functions is typical for a functionally oriented programming style.
...
In Listing \ref{fluximportR} it can be noticed that the numbers are passed as integers (e.g., ``\inlinecode{4L}'' instead of simply ``4'').
...
If, e.g., floating point numbers of type \inlinecode{Float64} were passed, the \inlinecode{Dense} function would yield a different result.
...
It is also not possible to infer the type automatically, because results of different methods of a specific function, which are invoked for different argument types, can be completely different.
Therefore, a \apkg{JuliaConnectoR} user must care about the types of the arguments exactly if the user also needed to care about them when using the functions directly in Julia.
...
In Julia, tabular structures are not part of the standard library but are only available in separate packages.
In addition to defining a data type for holding tabular structure, the Julia package \apkg{DataFrames} \citep{juliaDataFramesGithub} offers tools for working with tabular data.
...
To unify the different approaches, the \apkg{Tables} package \citep{tables.jl} was created, which defines a minimal common interface.
...
Because the interface of the \apkg{Tables} package has been stable for a longer time, with version 1.0 released in February 2020, and because of its lightweight implementation, the \apkg{Tables} package was used as the basis for the support of tabular structures in the \apkg{JuliaConnectoR}.
...
This means, the dictionary \inlinecode{Dict(:x => [[1,2],"a"])} is considered to be a table by \apkg{Tables} (version 1.3.1).
...
In contrast to R, missing values are represented in Julia by the \inlinecode{missing} object, which is of type \inlinecode{Missing}. Accordingly, the vector \inlinecode{[1.0, missing]} is of type \inlinecode{Array\{Union\{Missing, Float64\},1\}}, while \inlinecode{[1.0, NaN]} is of type \inlinecode{Array\{Float64,1\}}.
...
In Section \ref{juliaconnectorDbmexample}, a more useful example for using callback functions is given.
...
This includes the possibility to interrupt a running program or to inspect intermediate output.
...
For this purpose, some examples for using the \apkg{BoltzmannMachines} package, shown in \ref{bmusage}, are translated to R.
Additionally, these examples show how DBMs can be trained in R via the \apkg{BoltzmannMachines} package.
...
Partitioned architectures can also be built by specifying the arguments for the different parts via \inlinecode{TrainLayer} objects.
...
The functions of the \apkg{BoltzmannMachines} package are sensitive with respect to the argument types and it is required that the numbers have the correct types.
...
Before the training, the data is split into training and test data (lines 1-3 in Listing \ref{lst:monfitdbmr}).
...
After this preparation of the data that is used later for monitoring, the training is performed via calling \inlinecode{monitored\_fitdbm} in lines 8-12 in Listing \ref{lst:monfitdbmr}.
...
For the use in R, the function names need to be enclosed with backticks (lines 10-11 in Listing \ref{lst:monfitdbmr}).
...
Such a function can, e.g., be passed to \inlinecode{fitrbm} as \inlinecode{monitoring} argument (line 21 in Listing \ref{callbackexample}). (For the arguments of \inlinecode{fitrbm}, see p.~\pageref{bms_fitrbm}).
...
The goal of synthetic data is to create data that captures the underlying structure of the original data but that is not linked to the individuals and can be used without or with less privacy restrictions compared to the original data.
...
In particular, I investigate how the number of participating sites and the numbers of samples at each site influence the quality of the synthetic data.
...
The resulting curves look similar to the ones shown in Figure \ref{fig:monitoring_bmplots} (p.~\pageref{fig:monitoring_bmplots}).
...
In addition to DBMs, it is also possible to train RBMs and DBNs like in the \apkg{Boltzmann\-Machines} Julia package.
...
\subsection{Features of the presented software}

The \apkg{BoltzmannMachines} Julia package provides a user-friendly implementation of DBMs (see Section \ref{bmsjl}).
It is registered in the official package repository for the Julia language and provides several unique features, which are not covered by other software (see Section \ref{BMfeatures}).

One of these features is that one can easily create DBMs with partitioned architectures, which can help to train models on data of high dimensionality \citep{hess2017partitioned}.
The concept of multimodal DBMs goes further and allows different input distributions at the visible layer. 
This becomes possible by using restricted Boltzmann machines (RBMs) as simple building blocks, from which multimodal and partitioned architectures can be constructed.
The \apkg{BoltzmannMachines} package implements several different types of RBMs that can be used, including a new approach for categorical data (see Section \ref{methodsoftmax0}).
The possibility to customize the input layer has also been exploited by \cite{treppner_making_2020}, who constructed DBMs with input variables following negative binomial distributions for clustering single-cell sequencing data.

Other unique features of the \apkg{BoltzmannMachines} package concern the evaluation of the likelihood of DBMs and RBMs.
The likelihood and the variational lower bound of the likelihood can be estimated via annealed importance sampling (AIS, see Section \ref{evaluatingbms}). 
The AIS algorithm can be generalized on multimodal DBMs (see Section \ref{unnormalizedprobsrbm}) and it is available for all types of Boltzmann machines that can be constructed with the \apkg{BoltzmannMachines} package.
The exact calculation and the estimation of the likelihood of DBMs and RBMs are used to ensure the quality of the software via an extensive test suite.
More importantly, evaluating the training of DBMs is necessary for finding good hyperparameters.
Learning curves that can be generated from evaluating the models during the training.
This monitoring information can help in the choice of hyperparameters (see Section \ref{monitoring}).
In addition to standard evaluation metrics such as the (lower bound of the) likelihood or the reconstruction error (see Section \ref{alternativebmevaluations}), the monitoring of the training is fully customizable and can be used with any kind of evaluation (see Section \ref{juliaconnectorDbmexample} for an example).
This allows users to examine all possible aspects of RBM and DBM training.

The \apkg{BoltzmannMachines} package also allows a convenient access to the distribution of the hidden nodes.
Sample values for the hidden nodes can be generated by Gibbs sampling in the network, also conditioned on values for the visible units.
This could be exploited by \cite{hess_exploring_2020} to extract patterns in omics data, using DBMs as a dimension reduction technique.

The \apkg{BoltzmannMachines} package can also be used conveniently in R via the the \apkg{Julia\-ConnectoR} R package (see Section \ref{juliaconnectorDbmexample}).
R is a popular language for bioinformatics and biostatistics \citep{bioconductor}.
Moreover, there is no R package that allows to train DBMs.
Therefore it is an important asset if the DBM algorithms become also available to R users.
The \apkg{JuliaConnectoR} offers a generic solution to import functionality from Julia packages into R (see Section \ref{juliaconnectoRAutoimport}), which is not limited to the \apkg{Boltzmann\-Machines} package.
This ability to make the functions of complete Julia packages directly available in the form of R functions is one of the unique features that distinguishes the \apkg{JuliaConnectoR} from the other approaches for integrating Julia into R, namely \apkg{JuliaCall} \citep{JuliaCallPaper} and \apkg{XRJulia} \citep{chambersExtendingR}.
Furthermore, the \apkg{Julia\-ConnectoR} facilitates the interactive use by allowing to interrupt calls to Julia without threatening the stability of the R session.
This is particularly important for experimenting with deep learning algorithms, which may sometimes need an unexpectedly long time to complete (see Section \ref{juliaconnectorinterrupting}).
Another important feature for the interactive use is the redirection of output, which is also useful for getting feedback from long-running tasks (see Section \ref{juliaconnectorOutput}).
The well-documented concept for the translation of Julia and R types and objects between the two languages (see \ref{translatejuliar} and Appendix \ref{JuliaConnectoRDoku}, p.~\pageref{juliaconnectordokutypetranslation}ff.) further helps to create cross-language code.

Using the \apkg{JuliaConnectoR}, it is possible to wrap the  \apkg{Boltzmann\-Machines} package and make its functionality available in form of the DataSHIELD R package \apkg{dsBoltzmann\-Machines}.
This allows to use create synthetic data from biomedical or epidemiological data that can be distributed among different clinics/sites and that is subject to data protection constraints.
The \apkg{dsBoltzmann\-Machines} package is also the first solution that enables deep learning in DataSHIELD.
...
Major breakthroughs in language processing and image recognition such as the famous models {\em GPT-3} \citep{gpt3} and {\em ResNet} \citep{resnet} build on very deep architectures with many parameters.
...
This demonstrates that deep learning can also be feasible and beneficial without investments in special hardware.
...
Improvements such as reusing allocated space (see Section \ref{impltechnology} for more details) can be implemented directly in Julia.
...
For genetic variant data, an approach using sparse regression for partitioning clusters of SNPs has been examined in DBMs \citep{hess2017partitioned}, which was also implemented using the \apkg{BoltzmannMachines} package, and lead to meaningful results.
...
For this, a new approach is presented here, which evaluates the disclosure risk by assessing the overfitting via the proposed ``proportion of overfitting'' (see Section \ref{proportionoverfitting}).
...
If data is to be released completely without restrictions, e.g., by putting them on an internet site for anyone to download and analyze, the privacy concerns are higher than in a setting where only known users analyze data via DataSHIELD.
...
Yet, this applies regardless of the data protection method, as all approaches for managing the disclosure risk work with thresholds, which are hard to validate and also cannot give an absolute certainty (see Section \ref{privacyconcepts}).
...
Translating this idea to increase the quality of synthetic data could therefore be promising, particularly in settings of small sample size, where the issue of overfitting is most important.
...
Experimentally this could be investigated with a similar setup to the one in the distributed experiments (see Section \ref{simuexp} and Section \ref{realexp}).
...
This allows wrapping the functionality of the \apkg{BoltzmannMachines} package into a DataSHIELD R package for performing deep learning on distributed biomedical data under data protection constraints.
...
This appendix lists the documentation for all exported items of the \apkg{dsBoltzmannMachinesClient} package, version 1.0.2.
...
Freiburg, den 03.05.2021 \hspace{3cm} Stefan Lenz
%TODO Datum ggf. anpassen

\clearpage
\section{Danksagung}
Zuallererst möchte ich mich bei Prof.~Dr.~Harald Binder für das in mich gesetzte Vertrauen und den unermüdlichen Einsatz für die Wissenschaft bedanken, der die Bedingungen geschaffen hat, dass diese Arbeit möglich wurde.

Prof.~Dr.~Hans-Ulrich Prokosch möchte ich für die Bereitschaft, als zweiter Betreuer zu fungieren, danken. Als Leiter des MIRACUM-Konsortiums gilt ihm mein Dank natürlich auch für das Engagement in der Medizininformatik-Initiative.
Allen weiteren MIRACUM-Kollegen möchte ich hier ebenso Danke sagen.

Bei Dr.~Moritz Hess möchte ich mich für die fachlichen Diskussionen und ganz besonders auch für die persönliche Freundschaft in den letzten Jahren bedanken.

Bei allen ehemaligen und derzeitigen Mitgliedern der AG Machine Learning und der AG Studien und Beratung am IMBI Freiburg möchte ich mich für die angenehme und konstruktive Arbeitsatmosphäre bedanken.

Ich möchte mich auch noch bei Prof.~Dr.~Frank Ückert und Dr.~Torsten Panholzer für die für mich unschätzbar wertvolle Zeit am IMBEI in Mainz bedanken. Ohne diese Gelegenheit zur fachlichen und persönlichen Weiterentwicklung wäre es nicht zu dieser Arbeit gekommen. Auch bei allen anderen ehemaligen Kollegen am IMBEI in Mainz möchte ich mich für diese unvergessliche Zeit, die ich nicht missen möchte, bedanken.

\selectlanguage{UKenglish}
I would also like to take this opportunity to express my appreciation for the energy that the DataSHIELD team, led by Prof Paul Burton, is putting into developing usable software for biomedical research. I also want to thank the DataSHIELD team for their efforts in building a community around the product.

\selectlanguage{ngerman}

Für das aufmerksame Korrekturlesen möchte ich mich schließlich noch bei Maren Hackenberg und Dr.~Moritz Hess bedanken.
