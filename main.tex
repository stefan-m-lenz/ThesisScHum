...
Another possible application of generative models is to create synthetic data for privacy-preserving analyses of data that cannot be pooled due to data protection concerns.
This applies, e.g., to the MIRACUM consortium \citep{prokosch_miracum_2018}, which includes ten German University hospitals.
One of the goals there is to jointly analyze data that is distributed across multiple sites.
Pooling the data of all patients for joint analysis is inhibited by data protection laws.
Therefore, alternative ways of analyzing the data must be sought. 
The DataSHIELD software \citep{gaye_datashield,budin-ljosne_datashield}, which 
...
The question is here, whether deep learning techniques can be used to create synthetic data and whether they pose an increased risk with respect to violations of privacy when generating synthetic data.
