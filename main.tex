...
Thus, approaches that can yield good results also with a limited amount of data are needed in many cases when applying deep learning on biomedical data.
...
\begin{table}[h!]
\centering
\begin{tabular}{p{6.1cm} p{3cm} p{2.1cm} p{1.9cm} l}
%\hline
{\bf Feature} & {\bf \apkg{JuliaConnectoR}} & {\bf \apkg{JuliaCall}} & {\bf \apkg{XRJulia}} & {\bf See} \\
%\hline
...
\ref{RtoJulia} \\
Reversible translation & Yes
& No & No & \ref{JuliatoR} \\
Callbacks & Yes & Yes & No & \ref{Callbacks} \\
Let-syntax & Yes & No & No & \ref{LetSyntax} \\
Show standard (error) output & Yes & No & Yes &
\ref{OutputRedirection} \\
Interruptible & Yes & No & Yes & \ref{Interrupting} \\
Missing values & Yes & Yes & No & \ref{MissingValues} \\
\proglang{R} data frames to \proglang{Julia} Tables & Yes & Yes & No &
\ref{DataFrameSupport} \\
%\hline
\end{tabular}
...
There multiple models are trained on the data and their predictions are combined to get a better predictor.
...
Using the example of neural differential equations models \citep{juliaconnector_2021, chen_neural_2019}, it has been demonstrated that also complex neural networks based on \apkg{Flux} can be wrapped via the \apkg{JuliaConnectoR} for being handled in R.
...
By the same mechanism, VAEs or other complex generative models could be integrated in DataSHIELD for enabling distributed analyses based on synthetic data.
...
Lenz, S., Hackenberg, M. \harvardand\ Binder, H.  \harvardyearleft
  ...
  \newline(Accepted by {\em Journal of Statistical Software})
...
The contributions to the publications are as follows:
