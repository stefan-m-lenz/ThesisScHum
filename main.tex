...
As mentioned before in \ref{juliaconnectoRAutoimport}, a Julia function is translated by the \apkg{JuliaConnectoR} to an R function that invokes the Julia function.
These special R functions can be translated directly back to the original Julia functions.
Normal R functions are translated as callback functions.
That means, they are translated to Julia functions that invoke the original R functions.
Listing \ref{rfunascallback} and Listing \ref{juliafunascallback} show examples for this.
...
\begin{lstlisting}[language=R, float =!h, caption={Passing a reference to Julia function via the \apkg{JuliaConnectoR}},label={juliafunascallback}]
