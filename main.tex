...
In this  ``mean-field approximation" $q$, all hidden nodes are assumed to be independent.
...
\]
$E$ is the energy in the DBM and $\mathcal{H}(\mu) = \sum_j \left( \mu_j \log \mu_j + (1- \mu_j) \log ( 1- \mu_j) \right)$ denotes the entropy of the hidden nodes \citep{sala2012anefficient, salakhutdinov2015generativemodels}.

To calculate $\mu$ for a given data vector $\tilde{v}$, the following fix-point iteration can be used:
