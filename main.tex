...
Yet, there are some challenges that prevent deep learning from providing in bioinformatics applications. 
One big challenge in particular is limited data \citep{min_deep_2017}.
...
Although there is an abundance of routine care data, sample sizes are often small when it comes to curated data of clinical registries and clinical studies.
Additionally, privacy restrictions make it difficult to use health information of humans for research.

one class of deep learning approaches are generative models. 
DBMs are one of the first deep learning algorithms 
Generative models have made their appearance in mainstream media when an artist trained a generative model on images of paintings, used it to generate new paintings, and sold one of these paintings for \$432,000 at the auction house Christie's \citep{cohn_ai_2018}.


Besides generating data, DBMs and VAEs can be used for dimensionality reduction by .
The activation patterns of hidden nodes in DBMs and VAEs can be used to extract patterns in data \citep{hess_exploring_2020}.

Hpyerparameter is also a big challenge \citep{min_deep_2017}.
Since biomedical data sets can be very diverse, default values for hyperparameters may often not be the best choice.



DBMS good for small sample size \citep{nussberger_synthetic_2020}. 
Further partitioned can be used to improve on genetic data \citep{hess2017partitioned}.
...
Therefore partitioning \citep{hess2017partitioned}


Rare - distributed - privacy - DataSHIELD \citep{gaye_datashield,budin-ljosne_datashield}
TODO several approaches, e.g. synthpop. \citep{bonofiglio2020}. deep learning models as generative models capture more complicated structure
compare with no deep learning approaches. compare in a distributed setting.
Investigate with respect to privacy.
...
TODO aus motivation von Bioinf App not fail: VAE and GAN backpropagation, DBMs harder to train and the algorithm is complex.
...
Other popular approaches are also generative adversarial networks (GANs) \citep{goodfellow_generative_2014} and variational autoencoders (VAEs) \citep{Kingma2013}.
One advantage of DBMs is that it is particularly easy to perform conditional sampling, i.e., drawing from the learned distribution conditioned on certain values of input variables.
...
TODO link with small sample size. overall model. used for different stuff/transfer.

Summed up, deep Boltzmann machines show many promising qualities for deep learning on biomedical data, where limited sample size and privacy restrictions are common challenges.
This work aims to investigate this and to make DBMs available in form of user-friendly software.
...
A a first method for measuring disclosure, a membership attack \citep{choi_generating_2017, goncalves} is simulated.
...
\subsection{DBMs and other generative approaches}

\subsubsection{Performance on data of small sample size}
\cite{nussberger_synthetic_2020} have compared DBMs, VAEs, and GANs in more diverse settings, in particular with a broader range regarding the number of samples.
The experiment shown in section \ref{realexp} builds on this and performs this comparison in a setting of distributed data. 
Additionally to evaluating the performance, the disclosure is regarded here as well.
For this we used a new approach to look at the overfitting and the simulated membership attacks.

\subsubsection{Conditional sampling}



\subsubsection{Ideas for combining different models}

The findings here indicate that DBMs and VAEs both 
