...
\section*{Abstract}
TODO
%We present the Julia package \apkg{BoltzmannMachines} that provides a friendly interface to algorithms for training and evaluating Boltzmann machines.
...
Multimodal. Different data types. partitioned.
monitoring because all data sets different. hyperparameter selection important.
Based on Julia because fast and allows experimenting easily.
DBM because of their performance on data of small sample size.
In distributed analyses

Concept synthetic data distributed. genetic data. biomedical. privacy data protection.
Investigate the performance and the risk of disclosure of DBMs. 
For this, we compare DBMs with other popular generative approaches, namely generative adversarial networks (GANs) and variational autoencoders (VAEs).
...
This software is built in form of R packages that integrate into the DataSHIELD infrastructure for conducting privacy-preserving analyses on distributed cohorts.
...
Therefore, a new solution was created, which provides an a convenient interactive handling and at the same time puts a special focus on the stability that is needed for a production environment.
The \apkg{JuliaConnectoR} R package resulted as the product, which is officially registered in the Comprehensive R Archive Network (CRAN).
One of the unique features of the \apkg{JuliaConnectoR} is the ability to import arbitrary Julia packages in R such that functions and objects can be used and accessed directly in R.
As this approach does not rely on any specifics of DBMs or of the \apkg{BoltzmannMachines} package, this proves also on the technical side that it is possible to make advanced deep learning algorithms available for distributed privacy-preserving analyses in DataSHIELD.
...
These 30 genomic locations spanning 50 variables form 30 different data sets, which are used in the evaluation.
For each of these 30 different data sets, a training data set, a test data set, and a validation data set is selected.
