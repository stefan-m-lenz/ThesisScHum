...
These results strengthen the assumption that it is possible to use DBMs and VAEs for generating synthetic data from sensitive data of individuals.
...
\noindent\begin{minipage}{\linewidth}
   ...
   \captionof{figure}{Comparing the generative performance of DBMs, GANs, VAEs, IM, and MICE via the RMSE of log odds ratios. There is one data point for each data set in the box plots. (See \ref{realexpsetup} for the description of the data sets.)
...
\noindent\begin{minipage}{\linewidth}
   ...
   \captionof{figure}{Comparing the proportion of overfitting in DBMs, GANs, VAEs, IM, and MICE via the RMSE of log odds ratios in several settings of distributed data.
...
The previous experiments in \ref{simuexp} and \ref{realexp} have investigated the feasibility of using synthetic data for analyses.
In \ref{simuexp}, a hierarchical clustering was explored as joint analysis, and in  \ref{realexp}, the similarity of odds ratios in the synthetic inspected, which can be seen an indicator for how well the synthetic data can be used in analyses like logistic regression models.
...
The resulting software is the first that allows generating synthetic data in DataSHIELD and enables the approach that is depicted in figure \ref{fig:syntheticdataprinciple}.
In figure \ref{fig:examplaryworkflow}, the workflow using the software is sketched in more detail using a single site.
...
To perform the complete workflow as shown in figure \ref{fig:examplaryworkflow}, the following code can be executed in R.
In a first step, the user has to load \apkg{dsBoltzmannMachinesClient} package and login to the remote Opal servers.
...
Calling functions via this login object sends the request to multiple servers at once and  combines the result into one returned object. 
...
The login needs to be passed to the functions of the \apkg{dsBoltzmannMachinesClient} package to access the remote servers.
...
Via a call to \inlinecode{ds.monitored\_fitdbm}, a DBM can be fit on the training data.
Using both the training and the test data to evaluate the 
\begin{lstlisting}[language=R, float=!h]
...
The model is assigned to a variable at the server side, by default it is called ``\inlinecode{dbm}''.
Subsequent functions for using the model will also assume this as a default.
...
These measurements shows empirically, what is expected theoretically, i.e., that the algorithm has the same time complexity as matrix multiplication.
