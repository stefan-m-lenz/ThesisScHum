...
TODO gradient descent formula

An alternative is estimating the term with Gibbs sampling. The Gibbs sampling \citep{gibbssamplingorig} algorithm in RBMs can be formulated as follows:

\begin{enumerate}
\item Start with an arbitrary $v_1$.
\item Draw $h_1$ according to $p(h_1 \mid v_1)$.
\item Draw $v_2$ according to $p(v_2 \mid h_1)$.
...
In CD, the number of Gibbs sampling steps is reduced drastically. In most cases, only one step is used, which is denoted with $\text{CD}_1$ \citep{hinton_practical_2012}.
...
In an RBM with Bernoulli distributed nodes, this leads to the following formulas for the negative gradients for the weights and biases:
\begin{align*}
