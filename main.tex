...
One advantage of DBMs is that it is particularly easy to perform conditional sampling (see \ref{condsamplingrbm} and \ref{dbmprobs}), i.e., drawing from the learned distribution conditioned on certain values of input variables.
...
In both these approaches, the variables that are used as conditions need to be specified in the training.
In DBMs, this is not necessary and conditions can be set on arbitrary variables.
...
Another idea to create better synthetic data sets to increase the generative performance could to combine different types of generative models in ensembles of generative models .
The idea is similar to the idea of ensemble learning and super learners that combine different predictive models to get better predictions \citep{van_der_laan_targeted_2011}.
Generative ensembles have been studied in semi-supervised learning and TODO \citep{zanda_ensemble}.
For creating synthetic data, the ensembles could be used to enhance the performance with respect to a performance criterion that is applied on combined data from different models.
This is basically the same procedure as described in the experiments for (see figure \ref{fig:experimentalsetup} on p. \pageref{fig:experimentalsetup}), only with the same data but different generative approaches used at the  
In addition to that, a disclosure metrics could be applied.
This way, the different models could also be used to increase the diversity in the synthetic data.
