...
\begin{table}[h!]
\centering
\begin{tabular}{p{6.1cm} p{3.02cm} p{2.05cm} p{1.9cm} p{0.8cm} l}
%\hline
...
\subsubsection{Automatic importing of packages and modules}

\begin{lstlisting}[language=Julia,float=!h,caption={Julia code for importing the \apkg{Flux} package and defining a small neural network},label={fluximport}]
import Pkg
...
\begin{lstlisting}[language=R,float=!h,caption={R code using the \apkg{JuliaConnectoR} for  doing the same as in Listing \ref{fluximport}}]
Pkg <- juliaImport("Pkg")
Pkg$add(name = "Flux", version = "0.11")
