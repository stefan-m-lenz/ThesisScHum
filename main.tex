...
The algorithm for fitting a MICE model on binary data consists of the following steps \citep{goncalves, dsdbm}:
\begin{enumerate}
...
As simplest approach we use the method of {\em independent marginals} (IM) like \cite{goncalves}.
...
For Bernoulli distributed variables, this means to sample them using their estimated marginal means, as described above in the algorithm of the MICE method, where this is only used for sampling the first variable.
With this, the IM method provides a baseline comparison.
