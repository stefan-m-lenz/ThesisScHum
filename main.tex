...
Many deep learning frameworks such as \apkg{TensorFlow} \citep{tensorflow} and \apkg{PyTorch} \citep{pytorch} support to run code on graphics processing units (GPUs) in addition to the central processing unit (CPU).
Also the Julia package \apkg{Flux} supports to run array operations also on NVIDIA GPUs via the \apkg{CUDA} Julia package \citep{juliagpu}.
...
Here one needs also to take into account that the size of the matrices used in the training depends on the chosen batch size, as the update in the training algorithm can only be parallelized for samples in the same batch.
Specific hardware requirements such as restrictions to certain types of graphics cards of a specific vendor make this kind of optimizations less attractive, in particular when considering the diverse computing infrastructure in research environments.
Given these considerations, which show that it is not trivial to harness the multi-threading power of GPUs, a definitive use case where the performance would be needed to investigate this direction further.
