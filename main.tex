...
Also for clinical data, the availability of high-quality data is also one of the key challenges \citep{machine_learning_review_nejm}.
...
This makes unsupervised learning particularly interesting if there is not enough labeled data.
One class of unsupervised deep learning approaches are {\em generative models}, which are able to generate new data according to a distribution that is learned from input data.
...
The advantage of this generated synthetic data is that it can be used with less data protection restrictions of the original data as it does not contain individual-level data.
...
Such a model that has learned the overall structure of a data set is then also able to utilize the information about the whole distribution when making inferences about about smaller subgroups.
...
As can be seen Figure \ref{fig:dbmtrainingtimes} (p.\ \pageref{fig:dbmtrainingtimes}), the performance of DBM training scales in the same way as matrix multiplication.
For the measurements shown there, a virtual machine on a single core was used.
If multiple cores are involved, multi-threading can speed up the matrix multiplications for large matrices.
...
Measured on a single core in a virtualized environment on a standard desktop computer, the times give a lower bound of the computational performance.
...
There, standard hardware can be totally sufficient.
...
As the multiplication of large matrices is already multi-threaded, performance gains may only be expected if the matrix operations exhaust the threading capabilities of the CPU.
To get substantial performance improvements, the matrices need to have a sufficient size \citep{performancegpucpu}.
...
The experiment shown in section \ref{realexp} builds on \cite{nussberger_synthetic_2020} where DBMs, VAEs, and GANs are compared in diverse settings, in particular with a broad range regarding the number of samples.
The results of \cite{nussberger_synthetic_2020} also show that DBMs and VAEs are feasible generative models for genetic variant data, while the generative performance GANs is insufficient in settings of low sample size.
Here, these approaches are investigated in settings where data is distributed across a number of sites.
The results for the generative performance are in concordance with \cite{nussberger_synthetic_2020}.
...
For genetic variant data, an approach using a sparse regression that is used to partition based on clusters of SNPs has been examined in DBMs \citep{hess2017partitioned} and lead to meaningful results.
The implementation was also based on the \apkg{BoltzmannMachines} package.
Its concept of multimodal DBMs (see Figure \ref{mdbmimplasstack} on p.~\pageref{mdbmimplasstack}) allows to train DBMs with partitioned layers conveniently.


TODO multimodal implementation. martin?
...
Both disclosure metrics can be applied directly to the synthetic data and it is not required to distort the data or to modify the training to control the disclosure, as it is needed when applying differential privacy \citep{dwork_differential_2008, abadi_deep_2016}.

The membership attack requires to take the sensitivity and the threshold for the distance used by the attacker into account when interpreting the precision.
...
The proportion of overfitting has also an intuitive meaning, i.e., the amount of which a model learns more about the training data than about the general structure of the underlying data distribution.
...
Particularly interesting is the higher variance in the precision of the membership attack on MICE compared to VAEs and DBMs (see Figure \ref{fig:membershipattack} on p.~\pageref{fig:membershipattack}).
One can interpret this as a higher disclosure risk of MICE compared to the two neural network approaches.
Via the distance-base membership, there is not more identifying information derivable from the synthetic data generated by a DBM or VAE than in a sequence of logistic regression models, such as in the MICE method.
...
If data is to be released completely without restrictions, e.g., by putting it on a internet site for anyone to download and analyze, the privacy concerns are higher than in a setting where known users analyze data via DataSHIELD.
...
But this applies regardless of the data protection method, as all approaches for managing the disclosure risk work with thresholds, which are hard to validate and also cannot give an absolute certainty (see \ref{privacyconcepts}).
...
Experimentally this could be investigated with a similar setup to the one in the distributed experiments (see Figure \ref{fig:experimentalsetup} on p. \pageref{fig:experimentalsetup}).
In contrast to splitting the data set in different parts as done here, the same data but different generative approaches can be used to create the parts of the synthetic data set.
...
The findings here indicate that DBMs and VAEs can both be auitable for data of small sample size.
...
This can in principle be done analogously to the implementation shown in section \ref{dsBoltzmannMachinesImpl}.
...
Using the example of neural differential equations models \citep{juliaconnector_2021, chen_neural_2019}, it has been demonstrated that also complex neural networks based on \apkg{Flux} can be wrapped via the \apkg{JuliaConnectoR} to be handled in R, 
This shows that it is possible to bring very diverse deep learning algorithms into the DataSHIELD infrastructure.

\subsection{Conclusion}
Deep learning can help to create synthetic data that preserves higher-level structures.
Both DBMs and VAEs are promising models for creating synthetic data in settings of small sample size, whereas GANs do not perform well in such settings.
Further research to work out the and maybe combine different models.
