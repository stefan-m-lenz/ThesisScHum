...
In order to work with the abstraction, users must learn this new programming style, and, e.g., define the algorithms via computational graphs.

For a scientific use, flexibility is more useful than speed.

The automatic differentiation does not work with the Gibbs sampling algorithm. DBMs training fundamentally different from training other types of neural networks.
knet \citep{knetjl}
mocha \citep{mochajl}

Parallelization with Julia on compute cluster very easy.


tensorflow.jl \citep{tensorflowjl}
theano \citep{theano}
