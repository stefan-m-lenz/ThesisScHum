...
\[
\theta^{(t+1)} = \theta^{(t)} + \frac{\epsilon}{b_l} \sum_{\tilde{v} \in B_l} \left ( \nabla_{\!\theta} \log p_{\theta} \left( \tilde{v}\right) \right ) \left (\theta^{(t)}\right)
\]
...
This allowed to calculate $\EX_{P_{\text{data}}} \nabla_{\!\theta} E(\tilde{v},h)$ from $\EX p(h | \tilde{v})$ (see formula (\ref{eqn:plugexpectedh})).
...
The reconstruction \citep{hinton_practical_2012} for a sample $\tilde{v}$ is calculated by using the conditional probabilities as deterministic ``activation potential".
With $f_h(h) := \EX_v p(h|v)$ and $f_v(h) := \EX_h p(v|h)$, the reconstruction $r(v)$ can be defined as $r(\tilde{v}) := f_v(f_h(\tilde{v}))$. The reconstruction error is then the distance between the sample $\tilde{v}$ and its reconstruction $r(\tilde{v})$, e.g. measured as absolute distance or quadratic distance.
