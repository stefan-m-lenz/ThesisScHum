...
This works because the iteration forms a Markov chain, which has the distribution of the model as its equilibrium distribution .
...
With $a_h(h) := \EX_v p(h|v)$ and $a_v(h) := \EX_h p(v|h)$, the reconstruction $r(v)$ can be defined as $r(\widetilde{v}) := a_v(a_h(\widetilde{v}))$. The reconstruction error is then the distance between $\widetilde{v}$ and $r(\widetilde{v})$, e.g. measured as absolute distance or quadratic distance.
