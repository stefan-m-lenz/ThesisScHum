...
The probability of the activation is determined by an energy function that assigns an energy to each possible activation in the network or lattice.
Constellations with lower energy have a higher probability.
Therefore, Boltzmann machines are a type of so-called ``energy-based" models \citep{ranzato_ebm}.

Neural networks have input nodes that receive input from data.
In the physics analogue, this corresponds to parts of the lattice that are influenced by a magnetic field.
This information input is encapsulated in the parameters of the model, which influence the energy function.
The parameters encapsulate the information about input distribution.
Boltzmann machines are able to draw or generate new samples from 
...
% TODO example with number of parameters in gene rbm
