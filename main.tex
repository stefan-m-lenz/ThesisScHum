...
One indicator for successful learning in Boltzmann machines, which is applicable in all cases of data, remains the model likelihood. 
% TODO likelihood calculation requires the partition function
%In most cases, the likelihood cannot be calculated exactly but it can only be estimated by stochastic algorithms like annealed importance sampling (AIS). So we also want to detail the extension of AIS on multimodal deep Boltzmann machines in this article.
...
Since I have not found this formula in the literature, I derive it here in detail.
The unnormalized probability of the hidden nodes in a \inlinecode{GaussianBernoulliRBM} (original parameterization) can be calculated as
...
\begin{align*}
   ...
   &= \exp \left( b^T h + \sum_{i=1}^{n_V} \frac{\frac{1}{2}(Wh)_i^2 + (Wh)_i a_i}{\sigma_i^2} \right ) \prod_{i=1}^{n_V}\left(\sqrt{2\pi} \sigma_i \right).
\end{align*}
