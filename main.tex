...
When developing Julia code, writing functions and inside a module, which is defined in one or more Julia files, is a good practice because it allows to easily replace the complete code in the Julia session with a single call to the \inlinecode{include} function in Julia.
...
Given that types are important in Julia, the translations of the types are clearly documented \apkg{JuliaConnectoR} (see p.~\pageref{juliaconnectordokutypetranslation}).
 It is also not possible to infer this automatically, because it is possible that the result of different methods is completely different.
