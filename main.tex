...
For training the higher layers, a deterministic activation $f_k(v) :=  \EX p_k(h|v)$, defined via the conditional probability in the $k$-th RBM model, is passed through the lower layers:
The second RBM is trained with the data set of all $f_1(\tilde{v}^{(i)})$, the third RBM is trained with the data set $f_2(f_1(\tilde{v}^{(i)}))$, and so on.
