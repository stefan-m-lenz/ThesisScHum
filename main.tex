...
It supports to create partitioned architectures, which can help to train models on data of high dimensionality \citep{hess2017partitioned}.
...
The \apkg{BoltzmannMachines} package implements several different types of RBMs that can be used for this, including a new approach for categorical data (see Section \ref{methodsoftmax0}).
\cite{treppner_making_2020} exploited the possibility to customize the input layer, and constructed DBMs with input variables following negative binomial distributions for clustering single-cell sequencing data.

Other unique features of the \apkg{BoltzmannMachines} package concern the evaluation of the likelihood of DBMs.
The possibilities for evaluating the DBM training are used to ensure software quality with an extensive test suite and can be used for hyperparameter tuning.
There are also for monitoring the training. TODO

Via the \apkg{JuliaConnectoR} R package, the \apkg{BoltzmannMachines} package can also be conveniently used in R.
With its flexible approach, it becomes possible (see Section \ref{juliaconnectorDbmexample}).
...
For the use in a setting with distributed sensitive data, the \apkg{BoltzmannMachines} can be wrapped into an DataSHIELD R package.
This is the first package to implement deep learning in DataSHIELD.
