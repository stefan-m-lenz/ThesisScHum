...
%TODO hinton original code
As there was no package for training DBMs that was very user-friendly and easily extensible, a new software implementation was started.
This section describes the features and characteristics of the existing software implementations.
Table \ref{tab:FeatureOverviewBoltzmann} summarizes the existing solutions and compares them to the \apkg{BoltzmannMachines} package.
...
The original implementation of DBMs\footnote{\url{http://www.cs.toronto.edu/~rsalakhu/DBM.html}} was built in MATLAB (TODO ref).
...
The \apkg{deepbelief} package \citep{deepbelief_github} provides an object-oriented implementation of RBMs and DBNs in Python, using the linear algebra library \apkg{NumPy} \citep{numpy}.
...
AIS in yell not generic \footnote{\url{https://github.com/yell/boltzmann-machines/blob/93ece3497816ccf08f687e3237007544268788e7/boltzmann_machines/dbm.py\#L902}}

The prominent machine learning package \apkg{scikit-learn} can train binary restricted Boltzmann machines with the \inlinecode{BernoulliRBM} module\footnote{\url{https://scikit-learn.org/0.23/modules/generated/sklearn.neural\_network.BernoulliRBM.html}}.
Similar to the \apkg{Boltzmann} Julia package, it offers as evaluation metric the pseudo-likelihood.

The implementation (\apkg{deepbelief}) \apkg{yell/boltzmann-machines} do not provide the possibility to learn the noise parameter $\sigma$ for the visible nodes.
This is difficult
...
The \apkg{darch} package uses the GPL-3 license \citep{gplv3}, which enforces that the code for derived software must also be provided as open source.
