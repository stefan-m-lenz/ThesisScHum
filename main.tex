...
For this, we can use the fact that the states in one layer are independent from the states of each non-adjacent layer to get the unnormalized probability $p_k^*(\tilde{h}) = p_k^*(h^{(1)}) \cdot p_k^*(h^{(3)}) \cdot \dots $ for the subset $\tilde{h} = \left(h^{(1)}, h^{(3)}, \dots\right)$ of a generated sample $x = \left( v, h^{(1)}, h^{(2)}, \dots \right)$ in an intermediate model. 
...
\begin{align*}
p^*( \tilde{h} ) =&  p^*(h^{(1)}) \cdot p^*(h^{3}) \\
= &e^{(b^{(1)})^T h^{(1)}} \prod_{j}  \left( 1 + \exp \left( (b^{(2)} + (W^{(1)})^T v + W^{(2)} h^{(3)})_j \right) \right) \cdot \\
&e^{(b^{(3)})^T h^{(3)}} \prod_j (1+ \exp((b^{(2)}+ W^{(3)} h^{(3)})_j))
\end{align*}

TODO stimmt nicht!!!!%finish/überprüfen!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
