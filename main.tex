...
In Julia tabular structures are not part of the standard library but are available in separate packages.
There are also different approaches to tables in Julia, for example with the DataFrames package, which  or 
The \apkg{Tables} package defines an interface which unifies the different approaches.
...
As can be seen in listing TODO, missing values are handled in Julia 
.
Values of \inlinecode{NaN} and \inlinecode{NA} are distinguished.
