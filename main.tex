...
Therefore, the resulting data is not properly encoded in general  because illegal combinations of dummy variables can occur and then sampled values cannot be mapped to the original categories any more.
...
A variable with $k$ categories is 
Here, I tried a slightly different variant. This variant corresponds to the technique for using dummy variables for categorical variables in regression models \citep{faraway_regression}. A categorical variables with $k$ categories is encoded in $k-1$ binary dummy variables.
