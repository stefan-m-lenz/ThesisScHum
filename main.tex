...
Instead of using classical object oriented inheritance that Java or C++ offer, inheritance relationships (triangle-shaped arrows in the UML diagram) with can be implemented in Julia using {\em multiple dispatch} of the functions.
...
Figure \ref{umlclassdiagram} shows that (multimodal) DBMs are modeled as arrays (i.e. one-dimensional vectors) of RBMs.
...
An optimization of the algorithms for DBMs that contain only Bernoulli distributed nodes, such as the standard DBM.
...
This is a vector of RBMs or  \inlinecode{PartitionedRBMs} can be used to combine different RBMs, which may have different types, to build stacks of partitioned RBMs that form (see figure \ref{mdbmimplasstack}) TODO.


All types of Boltzmann machines have the common abstract supertype \inlinecode{AbstractBM}.
The functions that are available for all different subtypes can be seen in \ref{umlclassdiagram} in the 
...
\inlinecode{fitrbm} and \inlinecode{fitdbm} for fitting a complete RBM or DBM model, respectively. These functions get as input a data set and all hyperparameters for fitting the model.
